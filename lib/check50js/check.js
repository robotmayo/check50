...
var run50 = {
    host: 'http://localhost',//"http://run-1242295192.us-east-1.elb.amazonaws.com",
    port: 40232
};

var tests = {};

function endConnection(response, code) {
...
// make a post request to run50
function postRun50(data, path, callbackParams, callback) {
    var options = {
        host: run50.host,
        port: run50.port,
        path: path,
    ...
    var req = http.request(options, function(res) {
        ...
        res.on('data', function(chunk) {
            ...
            if (body.length > 1e6) {
                ...
                return endConnect(callbackParams.response, 404);
            }
        });

        res.on('end', function() {
            // can check to see if any files failed to upload
            callback(JSON.parse(body), callbackParams);
        });
    });

    req.on('error', function(e) {
        console.log('Error establishing connection to run50');
        return endConnection(callbackParams.response, 404);
    });
...
function parseOutput(output, params) {
   console.log(output);
   params.response.end();
}

function handleRunTest(token, params) {
    var data = {
        cmd: params.test.run,
        homedir: token.id
    };

    postRun50(qs.stringify(data), '/run', params, parseOutput);
}


function handleUploadTests(POST, response, request) {
    POST['testIds[]'].forEach(function(id) {
        ...
        if (typeof test === 'undefined')
            return endConnection(response, 404);
        ...
        postRun50(data, '/upload', callbackParams, handleRunTest);
    });
}
...
http.createServer(function(request, response) {
    ...
    request.on('end', function() {
        ...
        handleUploadTests(POST, response, request);    
    });
}).listen('50501'); 
